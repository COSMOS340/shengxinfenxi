...
repo_dir <- normalizePath(
  Sys.getenv("FIG4_REPO_DIR", unset = "I:/shengxinfenxi/bioinformatics-reproduction/projects/pan-cancer-mono-mac"),
  mustWork = TRUE
)
request_dir <- file.path(repo_dir, "desktop_exchange", "requests", "20260704_fig4a_gse306459_formal_snatac")
output_dir <- Sys.getenv(
  "FIG4_OUTPUT_DIR",
  unset = file.path(repo_dir, "desktop_exchange", "uploads", "20260704_fig4a_gse306459_formal_snatac", "pc_formal_fig4a_results")
)
...
if (any(!required_table$nonempty)) {
  missing <- required_table[nonempty == FALSE, relative_path]
