...
compute_gene_activity_from_peaks <- function(object, annotation_gr, target_genes = NULL) {
  ...
  if (!is.null(target_genes)) {
    ...
    keep <- gene_names %in% target_genes
    annotation_gr <- annotation_gr[keep]
    gene_names <- gene_names[keep]
    if (length(annotation_gr) == 0L) {
      stop("No annotation ranges remain after target gene filtering.")
    }
  }
...
fragment_list <- if (run_scope == "all") Filter(Negate(is.null), lapply(snatac_list, `[[`, "fragment")) else NULL
...
if (run_scope == "all") {
  annotation_gene_names <- unique(as.character(mcols(annotations)$gene_name))
  annotation_gene_names <- annotation_gene_names[!is.na(annotation_gene_names) & nzchar(annotation_gene_names)]
  gene_activity_targets <- intersect(reference_features, annotation_gene_names)
  if (length(gene_activity_targets) < 500L) {
    stop("Too few reference features overlap annotation genes for targeted gene activity: ", length(gene_activity_targets))
  }
  if (length(gene_activity_targets) > 3000L) {
    gene_activity_targets <- gene_activity_targets[seq_len(3000L)]
  }
  write_log("gene_activity", "running", paste0("fragments_free_peak_overlap; target_genes=", length(gene_activity_targets)))
  gene_activity <- compute_gene_activity_from_peaks(combined, annotations, target_genes = gene_activity_targets)
} else {
...
motif_lookup <- data.table(
  motif_id = names(pfm),
  motif_name = vapply(pfm, TFBSTools::name, character(1))
)
target_motifs <- c("TFEC", "TFE3", "BHLHE41", "BACH1", "Nfe2l2", "FOSL2")
selected_motifs <- motif_lookup[motif_name %in% target_motifs]
selected_motifs <- selected_motifs[match(target_motifs, motif_name)]
...
pfm_selected <- pfm[selected_motifs$motif_id]
write_log("chromvar", "running", paste0("motifs=", length(pfm_selected)))
BiocParallel::register(BiocParallel::SerialParam())
peak_counts_for_chromvar <- get_assay_data(combined, "peaks", "counts")
peak_gr_chromvar <- StringToGRanges(rownames(peak_counts_for_chromvar), sep = c(":", "-"))
genome(peak_gr_chromvar) <- "hg38"
chromvar_counts <- SummarizedExperiment::SummarizedExperiment(
  assays = list(counts = peak_counts_for_chromvar),
  rowRanges = peak_gr_chromvar
)
chromvar_counts <- chromVAR::addGCBias(
  chromvar_counts,
  genome = BSgenome.Hsapiens.UCSC.hg38::BSgenome.Hsapiens.UCSC.hg38
)
background_peaks <- chromVAR::getBackgroundPeaks(chromvar_counts)
...
chromvar_dev <- chromVAR::computeDeviations(
  object = chromvar_counts,
  annotations = motif_matches,
  background_peaks = background_peaks
)
chromvar_data <- SummarizedExperiment::assay(chromvar_dev, "z")
rownames(chromvar_data) <- selected_motifs$motif_id
colnames(chromvar_data) <- colnames(combined)
combined[["chromvar"]] <- CreateAssayObject(data = chromvar_data)
peak_counts_for_chromvar <- NULL
chromvar_counts <- NULL
background_peaks <- NULL
motif_matches <- NULL
chromvar_dev <- NULL
gc(verbose = FALSE)
write_log("chromvar", "complete", paste0("motifs=", nrow(chromvar_data), "; cells=", ncol(chromvar_data)))
...
motif_umap_plots <- Map(
  function(plot, label) plot + ggtitle(label),
  motif_umap_plots,
  selected_motifs$motif_name
)
p_motif_umaps <- wrap_plots(motif_umap_plots, ncol = 3)
