...
cell_list_paths <- list(
  broad_clean_singlets = file.path(request_dir, "inputs", "gse164522_broad_clean_singlet_cells.tsv"),
  core_clean_singlets = file.path(request_dir, "inputs", "gse164522_core_clean_singlet_cells.tsv")
)
...
run_one_set <- function(set_name, cell_path) {
  ...
  clean_obj <- RunUMAP(clean_obj, dims = dims_use, seed.use = 20260706, verbose = TRUE)
...
fwrite(combined_summary, file.path(output_dir, "gse164522_cleaned_recluster_combined_summary.tsv"), sep = "\t")

writeLines(
  c(
    "# GSE164522 cleaned macrophage/monocyte recluster",
    "",
