...
guest_free_bytes=$(df -B1 --output=avail /work | tail -n 1 | tr -d ' ')
printf 'checked_at\tcontext\tvolume\tobserved_free_bytes\trequired_free_bytes\tgate_passed\n' \
    > "$audit_dir/full_bamtofastq_disk_audit.tsv"
printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$(date --iso-8601=seconds)" \
    'GUEST_BEFORE_CONVERSION' \
    '/work' \
    "$guest_free_bytes" \
    "$disk_gate_bytes" \
    "$([[ $guest_free_bytes -ge $disk_gate_bytes ]] && echo True || echo False)" \
    >> "$audit_dir/full_bamtofastq_disk_audit.tsv"
if (( guest_free_bytes < disk_gate_bytes )); then
    echo "Guest disk gate failed: $guest_free_bytes < $disk_gate_bytes" >&2
    exit 31
fi
...
command_text="$bamtofastq_path --reads-per-fastq=1000000 $bam_path $output_dir"
start_time=$(date --iso-8601=seconds)
disk_log="$audit_dir/full_bamtofastq_disk_footprint.tsv"
printf 'timestamp\toutput_bytes\tguest_available_bytes\n' > "$disk_log"
...
while kill -0 "$full_pid" 2>/dev/null; do
    printf '%s\t%s\t%s\n' \
        "$(date --iso-8601=seconds)" \
        "$(output_bytes)" \
        "$(df -B1 --output=avail /work | tail -n 1 | tr -d ' ')" \
        >> "$disk_log"
    sleep 10
done
wait "$full_pid"
...

{
    ...
    echo "LOCUS_ARGUMENT: NOT_PRESENT"
    echo "START_TIME: $start_time"
    echo "END_TIME: $end_time"
    echo "EXIT_CODE: $full_exit"
...
} > "$audit_dir/full_bamtofastq_command_log.txt"
